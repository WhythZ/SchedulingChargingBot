...
GameManager::GameManager()
{
	...
	//生成场景纹理
	InitAssert(GenerateSceneTexture(), u8"Failed To Genrate Scene Texture");
	#pragma endregion
}
...
bool GameManager::GenerateSceneTexture()
{
	#pragma region PrepareMapTexture
	//获取场景地图纹理图片
	mapTexture = ResourceManager::Instance()->GetTexturePool().find(TextureResID::Park16x16)->second;

	//查询并记录场景地图的纹理宽高的宽高
	int _sceneMapWidth, _sceneMapHeight;
	SDL_QueryTexture(mapTexture, nullptr, nullptr, &_sceneMapWidth, &_sceneMapHeight);
	...
	mapRect.x = (ConfigManager::Instance()->basicPrefab.windowWidth - _sceneMapWidth) / 2;
	mapRect.y = (ConfigManager::Instance()->basicPrefab.windowHeight - _sceneMapHeight) / 2;
	//宽高的值是绝对的（x和y坐标表示的点的位置是相对于窗口的），所以直接赋值即可
	mapRect.w = _sceneMapWidth;
	mapRect.h = _sceneMapHeight;
	#pragma endregion
